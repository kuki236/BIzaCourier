...
from vista.detallesPedido import DetallesPedido
...
class buscarPedido(tk.Tk):
    ...
    def buscar(self):
        # Aquí se abre la ventana de detalles del pedido y se pasa el código
        codPedido = self.entradaPedido.get()
        if codPedido:

            dPedido = DetallesPedido(codPedido)
            self.destroy()
            dPedido.mainloop()
              # Hace que la ventana de detalles esté en foco y sea modal
